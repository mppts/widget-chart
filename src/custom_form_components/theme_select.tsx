...
const ThemeItemWrapper = styled.div.attrs(applyDefaultTheme)<{
  isActive: boolean;
}>`
  border-radius: 4px;
  display: flex;
  ...
  &:hover {
    border: ${(props) =>
      `1px solid ${(props.theme as ITheme).palette.primary}`};
  }
  ${(props) => {
    if (props.isActive) {
      return css`
    ...
    >
      <ThemeSelectWrapper>
          ...
          {multiColorTheme.map((themeKey, index) => {
            ...
            return (
              <ThemeItemWrapper
                ...
                isActive={isActive}
              >
                {themesMap[themeKey].colors20.map((color) => (
                  <ThemeColorSpan style={{ background: color }} />
                ))}
              </ThemeItemWrapper>
            );
          })}
        ...
        <Typography variant="body4">
          {t(Strings.single_color_gradient_theme)}
        </Typography>
          ...
          {singleColorTheme.map((themeKey, index) => {
            ...
            return (
              <ThemeItemWrapper
                ...
                isActive={isActive}
              >
                {themesMap[themeKey].colors10.map((color) => (
                  <ThemeColorSpan style={{ background: color }} />
                ))}
              </ThemeItemWrapper>
            );
...
export const ThemeSelect = ({ value, onChange }: ThemeSelectProps) => {
  const options = themes.map((themeKey) => ({
    value: themeKey,
    label: themesTransMap[themeKey],
  }));
  return (
    <Select
      ...
      dropdownRender={
        <ThemeSelectBase onChange={onChange} defaultValue={value} />
      }
