import { Form } from "@topspace/components";
import {
  BasicValueType,
  FieldType,
  useCloudStorage,
...
import {
  CHART_TYPES,
  DATETIME_FORMATTER_TYPES,
  DATETIME_FORMATTER_TYPES_NAMES,
} from "./const";
  ...
  return useCallback(() => {
    ...
    const dimensions = fields.filter((field) => {
      if (
        ...
        (field.type === FieldType.MagicLookUp &&
          field.entityType === FieldType.Attachment)
      ) {
        return false;
      }
      return true;
    ...
    const metrics = fields.filter(
      (field) => field.basicValueType === BasicValueType.Number
    );
    return {
      dataSource: { view: viewId, filter: null },
      ...new EchartsColumn(StackType.None, meta.theme).getDefaultFormData(
        dimensions,
        metrics
      ),
    };
...
const WidgetChartBase: React.FC = () => {
  ...
  const [formData, setFormData, editable] = useCloudStorage(
    "FormData",
    getDefaultFormData
  );
  ...
  const configChart = useMemo(
    () => new chartType.class(chartType.stackType, meta.theme) as EchartsBase,
    [chartType, meta.theme]
  );
  ...
  const schema: any = useMemo(() => {
    ...
    const chartStructureFormJSON = configChart.getChartStructureFormJSON(
      dimensions,
      metrics
    );
    // Configure the form style for the corresponding chart JSON.
    const chartStyleFormJSON = configChart.getChartStyleFormJSON([
      { id: undefined, name: "  " } as any,
      ...dimensions,
    ]);
    return {
      type: "object",
      ...
      definitions: {
        seriesField: getGroupOrStackFormJSON(dimensions, chartType.stackType)
          .seriesField,
        isFormatDatetime: {
          type: "boolean",
  ...
  const onFormChange = (data: any) => {
    ...
    if (
      formData.chartStructure.chartType !==
      nextFormData.chartStructure.chartType
    ) {
      const chartType = ChartMap[nextFormData.chartStructure.chartType];
      const _defaultFormData = (
        new chartType.class(chartType.stackType, meta.theme) as EchartsBase
      ).getDefaultFormData(dimensions, metrics);
      ...
      if (
        chartType.stackType !== StackType.None &&
        !mergedFormData.chartStructure.seriesField
      ) {
        const restFields = [...dimensions, ...metrics].filter(
          (item) =>
            item.id !== mergedFormData.chartStructure.dimension &&
            item.id !== mergedFormData.chartStructure.metrics
        );
  ...
  return (
    <div ref={containerRef} style={{ display: "flex", height: "100%" }}>
      ...
      <FormWrapper
        openSetting={isShowingSettings && meta.runtimeEnv == RuntimeEnv.Desktop}
        readOnly={readOnly}
      >
        <Form
          formData={formData}
